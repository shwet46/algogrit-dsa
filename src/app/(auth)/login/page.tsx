...
const isUsername = (v: string) => /^[a-zA-Z0-9_]{3,20}$/.test(v);
...
const LoginSchema = z.object({
  identifier: z
    ...
    .refine((v) => isEmail(v) || isUsername(v), 'Enter a valid email or username'),
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!isEmail(raw)) {
        ...
        if (!unameDoc.exists()) {
          throw { code: 'auth/user-not-found' } as { code: string };
        }
        const data = unameDoc.data() as { emailLower?: string };
        if (!data.emailLower) {
          throw new Error('Please login with your email address for this account.');
        }
        emailToUse = data.emailLower;
      } else {
      ...
      >
        ...
        >
          ...
          >
            {[{
              key: 'identifier',
              icon: <UserIcon className="h-5 w-5" />, type: 'text', placeholder: 'Email or Username', value: identifier, onChange: (e: React.ChangeEvent<HTMLInputElement>) => setIdentifier(e.target.value)
            }, {
              key: 'password',
          ...
          >
            <p className="text-zinc-500 text-xs font-medium">© 2025 AlgoGrit DSA. All rights reserved.</p>
          </motion.div>
        </motion.div>
      </motion.div>
    </div>
  ...
  return (
    <div className="group relative">
      <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none z-10">
        <div className={`transition-colors duration-200 ${isFocused ? 'text-indigo-300' : 'text-zinc-300'}`}>{icon}</div>
      </div>
      <input
        type={type}
