...
export default function Notes() {
  ...
  const allTags = Array.from(new Set(problems.problems.flatMap((p) => p.tags))).sort();
  ...
  // Normalize various createdAt shapes (Firestore Timestamp, Date, string, number) to a Date
  const toDateSafe = (value: unknown): Date => {
    ...
    if (
      typeof value === 'object' &&
      value !== null &&
      // Some older data might have a toDate function without being an instance of Timestamp
      'toDate' in value &&
  ...
  const filtered = notes
    ...
    .filter((n) => selectedTags.length === 0 || selectedTags.some((t) => n.tags.includes(t)));
  ...
  return (
    <div className="relative min-h-screen mt-20 w-full mb-16 bg-black text-white py-8 px-1 xs:px-2 sm:px-4 md:px-8 lg:px-16">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {/* Empty State */}
        {!loading && notes.length === 0 && <NotesEmptyState onAdd={() => setShowAdd(true)} />}
        ...
        {!loading && notes.length > 0 && (
          <>
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <span className="text-xs text-zinc-400 bg-zinc-800 px-2 py-1 rounded-full">
                        {toDateSafe(n.createdAt).toLocaleDateString()}
                      </span>
                    </div>
            ...
            {/* Pagination */}
            {totalPages > 1 && (
              <NotesPagination currentPage={currentPage} totalPages={totalPages} setCurrentPage={setCurrentPage} />
            )}
            {/* Full Page Note View */}
